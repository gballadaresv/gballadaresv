...
} elseif ($columnaSel && !preg_match('/^(\*|[a-zA-Z0-9_]+)$/', $columnaSel)) {
    $error = "Columna no permitida.";

} elseif ($whereCampo && !preg_match('/^(\*|[a-zA-Z0-9_]+)$/', $whereCampo)) {
    $error = "Filtro no permitido.";

} else {
    try {
        $sql = "SELECT $columnaSel FROM $tablaSel";
        if ($whereCampo && $whereValor !== '') {
            $sql .= " WHERE $whereCampo = :where_valor";
        }
        $stmt = $db->prepare($sql);
        if ($whereCampo && $whereValor !== '') {
            $stmt->bindParam(':where_valor', $whereValor);
        }
        $stmt->execute();
