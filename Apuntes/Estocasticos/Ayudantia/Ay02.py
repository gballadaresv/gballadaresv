...
# Para mostrar gráficos en Colab
# %matplotlib inline

def simular_experimento():
    """
    Simula el experimento de lanzar una moneda hasta obtener 3 caras.
    Retorna el número de lanzamientos requeridos.
    """
    conteo = 0
    caras = 0
    while True:
...
probs_teoricas = [comb(n-1, 2) / 2**n for n in n_values]
...
plt.plot(n_values, probs_teoricas, marker='o', linestyle='-', color='darkred',
         linewidth=2, markersize=6, label=r'Teórico: $P(N=n)=\binom{n-1}{2}\frac{1}{2^n}$')
...
plt.savefig('Grafico_3Caras.png')
plt.show()
