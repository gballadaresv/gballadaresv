...
\today\\
\vspace{0.2cm}
\footnotesize{1º semestre 2024 - Profesores P. Bahamondes - S. Bugedo - N. Alvarado}\\
\vspace{0.2cm}
\footnotesize{\alumno}
...
\end{equation}
Por induccion simple. El caso donde $n=0$ se cumple de forma trivial.
Supondremos que (\ref{eq:1}) se cumlpe para un $n$ arbitrario y demostraremos que se cumple para el caso $n+1$.
\begin{align*}
    \sum_{i=0}^{n+1} i^2
    &= \sum_{i=0}^{n} i^2 + (n+1)^2 \\
    & \stackrel{HI}{=} \frac{n(n+1)(2n+1)}{6} + (n+1)^2 \\
    &= \frac{(n+1)\big((2n^2 + n) + 6(n+1)\big)}{6} \\
